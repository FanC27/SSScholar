...
def main(args):
    ...
    parser.add_option('--struct-model', dest='struct_model', type=str, default='t2vec',
                      help='Name of struct model')
    parser.add_option('--dataset', dest='dataset', type=str, default='NYC',
                      help='Name of dataset')
    parser.add_option('--batch-size', dest='batch_size', type=int, default=4000,
                      help='Size of minibatches: default=%default')
    parser.add_option('--epochs', type=int, default=2000,
    ...
    parser.add_option('--pos-train-prefix', dest='pos_train_prefix', type=str, default='pos_train',
                      help='Prefix of pos_train set: default=%default')
    parser.add_option('--neg-train-prefix', dest='neg_train_prefix', type=str, default='neg_train',
                      help='Prefix of neg_train set: default=%default')
    parser.add_option('--test-prefix', type=str, default=None,
                      help='Prefix of test set: default=%default')
    parser.add_option('--merge', dest='merge', action='store_true', default=True,
                      help='merge struct and activity or not?')
    parser.add_option('--no-merge', dest='merge', action='store_false', default=False)
    parser.add_option('--contrastive', dest='contrastive', action='store_true', default=True,
                      help='Using contrastive learning?')
    parser.add_option('--no-contrastive', dest='contrastive', action='store_false', default=False)
    parser.add_option('--mode', dest='mode', type=str, default='train',
                      help='train or test?')
    ...
    parser.add_option('--emb-dim', type=int, default=256,
    ...
    if options.seed is not None:
        ...
        rng = np.random.RandomState(np.random.randint(0, 100000)) # 设随机种子
        seed = None

    # load the training data
    train_X, vocab, row_selector, train_ids = load_word_counts(input_dir, options.train_prefix) # 得到[数据长度，词表长度], 词表token, 过滤器, 数据id

    pos_train_X, _, pos_row_selector, pos_train_ids = load_word_counts(input_dir, options.pos_train_prefix, vocab)
    neg_train_X, _, neg_row_selector, pos_train_ids = load_word_counts(input_dir, options.neg_train_prefix, vocab)
    struct_train_X = fh.get_struct_vec('train', options.struct_model, options.dataset, 'train', _, _, _)
    struct_test_X = fh.get_struct_vec('train', options.struct_model, options.dataset, 'test', _, _, _)
    train_labels, label_type, label_names, n_labels = load_labels(input_dir, options.train_prefix, row_selector, options) # none
    # None
    ...
    options.n_train, vocab_size = train_X.shape # 数据长度, 词表长度
    options.n_labels = n_labels # None

    if n_labels > 0: # None
        print("Train label proportions:", np.mean(train_labels, axis=0))

    # split into training and dev if desired 没有验证集
    ...
    n_train, _ = train_X.shape # 训练数据长度

    # load the test data
    if options.test_prefix is not None:
        test_X, _, row_selector, test_ids = load_word_counts(input_dir, options.test_prefix, vocab=vocab) # [测试数据长度，词表长度], 不需要再求词表, 过滤器, 数据id
    ...
    # initialize the background using overall word frequencies 通过训练数据整个的词频来初始化背景
    init_bg = get_init_bg(train_X) # 词表中的token在全文中出现的频率
    if options.no_bg:
    ...
    # load word vectors
    embeddings, update_embeddings = load_word_vectors(options, rng, vocab)  # 不使用word2vec，需要更新嵌入

    if options.mode == 'train':
        # create the model 搭建网络模块
        ...
        # train the model 反向传播求loss
        print("Optimizing full model") 
        model = train(model, network_architecture, options.contrastive, options.merge, train_X, pos_train_X, neg_train_X, struct_train_X, train_labels, train_prior_covars, train_topic_covars, training_epochs=options.epochs, batch_size=options.batch_size, rng=rng, X_dev=dev_X, Y_dev=dev_labels, PC_dev=dev_prior_covars, TC_dev=dev_topic_covars)
        ...
        # save_document_representations(model, options, train_X, pos_train_X, neg_train_X, struct_train_X, train_labels, train_prior_covars, train_topic_covars, train_ids, options.output_dir, 'train', batch_size=options.batch_size)

    elif options.mode == 'test':
        print("obtain the embedding!!!")
        if options.contrastive and options.merge:
            model = torch.load(f'./weight/{options.struct_model}/{options.dataset}/contrastive_merge_{options.n_topics}.pt')
            model.eval()
        elif options.contrastive:
            model = torch.load(f'./weight/{options.struct_model}/{options.dataset}/contrastive_{options.n_topics}.pt')
            model.eval()
        elif options.merge:
            model = torch.load(f'./weight/{options.struct_model}/{options.dataset}/merge_{options.n_topics}.pt')
            model.eval()
        else:
            model = torch.load(f'./weight/{options.struct_model}/{options.dataset}/original_model_{options.n_topics}.pt')
            model.eval()
        
        save_document_representations(model, options, test_X, struct_test_X, test_labels, test_prior_covars, test_topic_covars, test_ids, options.output_dir, 'test', batch_size=options.batch_size)
    ...
    # if test_X is not None: # 打印测试文本的困惑度
...
def load_word_counts(input_dir, input_prefix, vocab=None):
    ...
    X = fh.load_sparse(os.path.join(input_dir, input_prefix + '.npz')).tocsr() # 将稀疏矩阵映射为不含0的元素 [数据长度，词表长度]
    # load the vocabulary
    if vocab is None:
        vocab = fh.read_json(os.path.join(input_dir, input_prefix + '.vocab.json'))  # 读取所有的token，存为list
    n_items, vocab_size = X.shape # n_items = 数据长度, vocab_size = 词表长度
    assert vocab_size == len(vocab) # 判断词表长度是否相同
    print("Loaded %d documents with %d features" % (n_items, vocab_size))

    ids = fh.read_json(os.path.join(input_dir, input_prefix + '.ids.json')) # 按照顺序读取数据的id
    ...
    row_sums = np.array(X.sum(axis=1)).reshape((n_items,)) # 读取每行数据的token总数
    row_selector = np.array(row_sums > 0, dtype=bool) # 定义一个判断器，查看有没有哪一行没有数据
    # false_indices = np.where(~row_selector)[0] # 找到空元素的id
    # data = fh.read_json('/data/home/fcf/firstwork/ltc/scholar/data/poi/train_file.jsonlist')
    # print("IDs with no data:", false_indices)
    # print(data[false_indices])
    # print("Found %d non-empty documents" % np.sum(row_selector))
    # X = X[row_selector, :]
    # ids = [doc_id for i, doc_id in enumerate(ids) if row_selector[i]] # 更新id
...
def get_init_bg(data):
    ...
    n_items, vocab_size = data.shape # 数据长度，词表大小
    sums = np.array(data.sum(axis=0)).reshape((vocab_size,))+1. # 每个单词在全文中出现的次数 +1平滑
...
def train(model, network_architecture, contrastive, merge, X, pos_X, neg_X, struct_train_X, Y, PC, TC, batch_size=200, training_epochs=100, display_step=10, X_dev=None, Y_dev=None, PC_dev=None, TC_dev=None, bn_anneal=True, init_eta_bn_prop=1.0, rng=None, min_weights_sq=1e-7):
    # Train the model
    n_train, vocab_size = X.shape # 数据长度，词表长度
    mb_gen = create_minibatch(X, pos_X, neg_X, struct_train_X, Y, PC, TC, batch_size=batch_size, rng=rng)
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            # get a minibatch
            batch_xs, pos_batch_xs, neg_batch_xs, struct_batch_xs, batch_ys, batch_pcs, batch_tcs = next(mb_gen) # batch_xs -> [批次大小(id)，词表大小(每个词在本条数据的词频)]
            # do one minibatch update 开始训练，包括前向传播 反向传播 计算loss
            if contrastive:
                cost, recon_y, thetas, nl, kld, contrasitve_loss = model.fit(contrastive, merge, batch_xs, pos_batch_xs, neg_batch_xs, struct_batch_xs, batch_ys, batch_pcs, batch_tcs, eta_bn_prop=eta_bn_prop, l1_beta=l1_beta, l1_beta_c=l1_beta_c, l1_beta_ci=l1_beta_ci)
                avg_cl += float(contrasitve_loss) / n_train * batch_size
            else:
                cost, recon_y, thetas, nl, kld = model.fit(contrastive, merge, batch_xs, pos_batch_xs, neg_batch_xs, struct_batch_xs, batch_ys, batch_pcs, batch_tcs, eta_bn_prop=eta_bn_prop, l1_beta=l1_beta, l1_beta_c=l1_beta_c, l1_beta_ci=l1_beta_ci)
        ...
        if epoch % display_step == 0 and epoch > 0:
            if network_architecture['n_labels'] > 0:
                ...
                print("Epoch:", '%d' % epoch, "cost=", "{:.9f}".format(avg_cost), "contrasitve_loss=", "{:.9f}".format(avg_cl))
...
def create_minibatch(X, pos_X, neg_X, struct_X, Y, PC, TC, batch_size=200, rng=None):
    ...
    while True:
        ...
        pos_X_mb = np.array(pos_X[ixs, :].todense()).astype('float32')
        neg_X_mb = np.array(neg_X[ixs, :].todense()).astype('float32')
        struct_X_mb = np.array(struct_X[ixs, :]).astype('float32')
        if Y is not None:
        ...
        yield X_mb, pos_X_mb, neg_X_mb, struct_X_mb, Y_mb, PC_mb, TC_mb

# def create_minibatch(X, Y, PC, TC, batch_size=200, rng=None): # 按顺序选取训练数据
    count = 0
    data_length = X.shape[0]
    while True:
        if count + batch_size > data_length:
            count = 0
        X_mb = np.array(X[count:count + batch_size, :].todense()).astype('float32')
        if Y is not None:
            Y_mb = Y[count:count + batch_size, :].astype('float32')
        else:
            Y_mb = None

        if PC is not None:
            PC_mb = PC[count:count + batch_size, :].astype('float32')
        else:
            PC_mb = None

        if TC is not None:
            TC_mb = TC[count:count + batch_size, :].astype('float32')
        else:
            TC_mb = None
...
def get_minibatch(X, struct_X, Y, PC, TC, batch, batch_size=2200):
    ...
    return X_mb, struct_X_mb, Y_mb, PC_mb, TC_mb
...
def print_and_save_weights(options, model, vocab, prior_covar_names=None, topic_covar_names=None):
    ...
    # print background 输出所有文本中出现频率最高的词
    bg = model.get_bg()
    ...
    merge = options.merge
    fh.makedirs(f'./weight/{options.struct_model}/{options.dataset}')

    # 保存模型训练参数，共有四种不同情况(contrastive+merge, contrastive, merge,XX)
    if contrastive and merge:
        torch.save(model, f'./weight/{options.struct_model}/{options.dataset}/contrastive_merge_{options.n_topics}.pt')
    elif contrastive:
        torch.save(model, f'./weight/{options.struct_model}/{options.dataset}/contrastive_{options.n_topics}.pt')
    elif merge:
        torch.save(model, f'./weight/{options.struct_model}/{options.dataset}/merge_{options.n_topics}.pt')
    else:
        torch.save(model, f'./weight/{options.struct_model}/{options.dataset}/original_model_{options.n_topics}.pt')

    # print topics [topic_size, vocab_size]每个主题所对应单词在词表中的概率
    emb = model.get_weights()
    print("Topics:")
    maw, sparsity = print_top_words(emb, vocab) # 通过给概率排序，打印出概率最高的词和最低的词
...
def save_weights(output_dir, beta, bg, feature_names, sparsity_threshold=1e-5):
    ...
    topics2word_prob_file = os.path.join(output_dir, 'topic2word_prob.txt')
    lines = []
    vectors = []
    for i in range(len(beta)):
        order = list(np.argsort(beta[i]))
        order.reverse()
        top_100_indices = order[:3]
        sorted_indices = np.argsort(beta[i])
        top_100_values = beta[i][top_100_indices]
        normalized_values = softmax(top_100_values)
        output_prob = ' '.join([str(element) for element in normalized_values])
        pos_words = [feature_names[j] for j in order[:10] if beta[i][j] > sparsity_threshold]
        output = ' '.join(pos_words)
        lines.append(output)
        vectors.append(output_prob)

    fh.write_list_to_text(lines, topics_file)  # 写出每个主题的前十个词
    fh.write_list_to_text(vectors, topics2word_prob_file)  # 记录前n个词的概率分布
...
def save_document_representations(model, options, X, struct_X, Y, PC, TC, ids, output_dir, partition, batch_size=2200):
    # compute the mean of the posterior of the latent representation for each documetn and save it
    # if Y is not None:
    #     Y = np.zeros_like(Y)

    n_items, _ = X.shape # 数据大小
    n_batches = int(np.ceil(n_items / batch_size)) # batch次数
    thetas = []

    for i in range(n_batches):
        batch_xs, struct_batch_xs, batch_ys, batch_pcs, batch_tcs = get_minibatch(X, struct_X, Y, PC, TC, i, batch_size) # 得到一个batch的数据
        thetas.append(model.compute_theta(False, options.merge, batch_xs, struct_batch_xs, batch_ys, batch_pcs, batch_tcs))
    theta = np.vstack(thetas)
    fh.makedirs(f'{output_dir}/{options.struct_model}/{options.dataset}/{options.mode}')
    if options.contrastive and options.merge:
        np.savez(os.path.join(output_dir, options.struct_model, options.dataset, options.mode, f'theta_contrastive_merge_{options.n_topics}.npz'), theta=theta, ids=ids)
    elif options.contrastive:
        np.savez(os.path.join(output_dir, options.struct_model, options.dataset, options.mode, f'theta_contrastive_{options.n_topics}.npz'), theta=theta, ids=ids)
    elif options.merge:
        np.savez(os.path.join(output_dir, options.struct_model, options.dataset, options.mode, f'theta_merge_{options.n_topics}.npz'), theta=theta, ids=ids)
    else:
        np.savez(os.path.join(output_dir, options.struct_model, options.dataset, options.mode, f'theta_{options.n_topics}.npz'), theta=theta, ids=ids)
